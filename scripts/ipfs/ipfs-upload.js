...
const {
  validateThenConvertCsv,
} = require("./helper-scripts/process-metadata-csv");
const { validateImages } = require("./helper-scripts/process-image-folder");
const {
  validateAnimations,
} = require("./helper-scripts/process-animation-folder");
const fs = require("fs");
...
const uploadLocalFolderToIpfs = async (
  localFolderPath,
  ipfsFolderPath,
  chunkSize = 10,
) => {
  ...
  const processBar = createProcessBar(
    `Upload local folder (${localFolderPath}) to IPFS`,
    filenames.length / chunkSize,
  );
  ...
  const folderCid = (
    await ipfsClient.files.stat(ipfsFolderPath)
  ).cid.toString();
...
const uploadMetadataObjectToIPFS = async (
  metadataObjects,
  ipfsFolderPath,
  chunkSize = 10,
) => {
  ...
  const processBar = createProcessBar(
    ...
    metadataObjects.length / chunkSize,
  );
  for (let i = 0; i < metadataObjects.length; i += chunkSize) {
    const uploadPromisses = metadataObjects
      .slice(i, i + chunkSize)
      .map((metadataObject) => {
        let ipfsPath;
        let content;
        ...
        content = JSON.stringify(
          _.omit(metadataObject, ["filename", "token_id"]),
        );
...
const uploadToIPFS = async (
  ...
  localAnimationFolderPath,
  ipfsFoldername,
  overwrite = false,
) => {
  if (
    !ipfsFoldername ||
    _.isEmpty(metadataObjects) ||
    _.isEmpty(localImageFolderPath)
  )
    throw new Error("Not enough data to perform upload");
  ...
  const { cid: imageFolderCid, filenames: imageFilenames } =
    await uploadLocalFolderToIpfs(
      localImageFolderPath,
      `/${ipfsFoldername}/images`,
    );
  const { cid: animationFolderCid, filenames: animationFilenames } = !_.isEmpty(
    localAnimationFolderPath,
  )
    ? await uploadLocalFolderToIpfs(
        localAnimationFolderPath,
        `/${ipfsFoldername}/animations`,
      )
    : undefined;
  ...
  for (
    let i = 0, j = 0;
    i < metadataObjects.length && j < imageFilenames.length;
    i++
  ) {
  ...
  const medatadaFolderCid = await uploadMetadataObjectToIPFS(
    metadataObjects,
    `/${ipfsFoldername}/metadata`,
  );

  console.log(
    `\n\nUploaded metadata. Link: ${process.env.IPFS_LINK_PREFIX}${medatadaFolderCid}`,
  );
  const collectionFolderCid = (
    await ipfsClient.files.stat(`/${ipfsFoldername}`)
  ).cid.toString();
...
async function main() {
  ...
  if (!ipfsFoldername)
    throw new Exception(
      `Name of folder (used as IPFS upload destination) must be specified as the first argument of the script`,
    );
  ...
  const { metadataObjects, filenames } = await validateThenConvertCsv(
    metadataCSVFilePath,
    undefined,
    { reservedTokenIds: [], maxSupply },
  );
