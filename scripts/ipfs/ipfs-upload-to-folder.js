...
async function uploadFileToIPFSFolder(localDirPath, filename, ipfsDirName) {
  ...
  await ipfs.files.write(`/${ipfsDirName}/${filename}`, filedata, {
    create: true,
  });
}
...
async function uploadFolder(localDirPath, ipfsDirName) {
  ...
  filenames.sort((i1, i2) =>
    i1.localeCompare(i2, undefined, { sensitivity: "variant", numeric: true }),
  );
  ...
  if (uploadResponse.blocks !== numberOfFile) {
    ...
    throw new Error(
      `Not all file uploaded. Only ${uploadResponse.blocks}/${numberOfFile} files uploaded`,
    );
  }
