...
async function createOffer() {
  ...
  const wethContract = new ethers.Contract(
    networkConfig.zora_contract_addresses.WETH,
    wethABI,
    deployer,
  );
  const wethApproveResponse = await wethContract.approve(
    networkConfig.zora_contract_addresses.ERC20_TRANSFER_HELPER,
    offerPrice,
  );
  await sleep(1000);
